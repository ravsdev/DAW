...
public class Principal {
    ...
    private enum OPCIONES {
        ...
        ELIMINAR("Eliminar cuenta."), CLIENTES("Listado clientes"), SALIR("Salir.");
    ...
    //Constantes con los nombres de los archivos de nuestra aplicación
    private static final String FICHERODATOS = "datoscuentasbancarias.dat";
    private static final String FICHEROTEXTO = "ListadoClientesCCC.txt";
    ...
    private static final String VALIDOPTIONS = "Valores permitidos 1-9.";
    ...
    public static void main(String[] args) {
        //Obtenemos las cuentas guardadas, si las hay.
        recuperarCuentas();
    ...
    /**
     * Antes de lanzar el menú, comprobamos si hay cuentas guardadas y las
     * recuperamos.
     */
    @SuppressWarnings("unchecked")
    public static void recuperarCuentas() {
        ArrayList<CuentaBancaria> cuentasDB;
        cuentasDB = (ArrayList) Ficheros.leer(FICHERODATOS);

        //Si el fichero existe asignamos la lista de cuentas a nuestro banco.
        if (cuentasDB != null) {
            banco.setCuentas(cuentasDB);
        } else {
            //Si no hay cuentas mostramos al usuario un aviso.
            System.out.println("AVISO: No se encuentra ninguna cuenta.");
        }
    }
    ...
    public static void accion(int opcion) {
        switch (opcion) {
            ...
            case 7:
                ...
                break;
            case 8:
                //Llamamos al método listarClientes
                listarClientes();
                break;
            case 9:
                //Sale del programa
                //Guardamos las cuentas en un archivo para poder recuperarlas en un próximo uso del programa.
                Ficheros.escribir(FICHERODATOS, banco.getCuentas());
    ...
    /**
     * Guardamos en un fichero de texto el listado de clientes.
     */
    private static void listarClientes() {
        String[] clientes = banco.listadoClientes();
        
        //Si hay cuentas existentes, las exportamos a un fichero de texto
        if (clientes.length-1 > 0) {
            //Si el fichero se crea con éxito mostramos un mensaje por pantalla.
            if (Ficheros.crearTxt(FICHEROTEXTO, clientes)) {
                System.out.println("Archivo ListadoClientesCCC.txt creado con éxito.");
            }
        } else {
            //Si no hay cuentas, NO generamos ningún archivo y mostramos un mensajes de aviso.
            System.out.println("No hay cuentas que exportar.");
        }
    }
    ...
    /**
     * Pedimos al usuario un número de cuenta, comprobamos si existe e
     * intentamos eliminarla. El método eliminarCuenta de la clase Banco nos
     * devolverá TRUE si se ha podido eliminar o FALSE en caso contrario.
     */
    private static void eliminarCuenta() {
        ...
        if (!banco.comprobarCuenta(iban)) {
            System.out.println("= Cuenta no encontrada. =");
        } else if (banco.eliminarCuenta(iban)) {
            System.out.println("= Cuenta eliminada. =");
        } else {
            System.out.println("La cuenta tiene saldo, debe retirar el saldo antes de eliminarla.");
        }
    }
    ...
    private static float obtenerInteres() {
        ...
        do {
            ...
            if (interes > 100) {
                System.out.println("El interés no puede ser mayor a 100.");
            }
        } while (interes > 100);
    ...
    private static float obtenerComision() {
        ...
        do {
            ...
            if (interes > 100) {
                System.out.println("La comisión no puede ser mayor a 100.");
            }
        } while (interes > 100);
