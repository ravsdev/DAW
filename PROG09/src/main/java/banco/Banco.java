...
public class Banco {
    ...
    /**
     * Devuelve un array de cadenas de texto con todas los clientes y su número
     * de cuenta.
     *
     * @return Array de cadenas de texto
     */
    public String[] listadoClientes() {
        int numCuentas = cuentas.size();
        String[] listado = new String[numCuentas + 1];
        ...
        for (i = 0; i < numCuentas; i++) {
            ...
            listado[i] = String.format("%s"
                    + "\n"
                    + "IBAN: %s"
                    + "\n", cuentas.get(i).getTitular().devolverInfoString(), cuentas.get(i).getNumCuenta());
        }
        listado[i] = String.format("Número de cuentas existentes: %s", numCuentas);
    ...
    /**
     * Devuelve una copia de las cuentas.
     * @return Devuelve una copa de las cuentas.
     */
    ...
    /**
     * Establece las cuentas del banco.
     * @param cuentas Establece las cuentas del banco.
     */
    @SuppressWarnings("unchecked")
    public void setCuentas(ArrayList<CuentaBancaria> cuentas) {
        this.cuentas = (ArrayList<CuentaBancaria>) cuentas.clone();
    }
}
